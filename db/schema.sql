...
CREATE TABLE IF NOT EXISTS `capstone_scheduling_project`.`time_blocks` (
  ...
  `corresponding_block` INT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE INDEX (`day` ASC, `timeslot` ASC) VISIBLE,
  INDEX `fk_time_blocks_time_blocks1_idx` (`corresponding_block` ASC) VISIBLE,
  CONSTRAINT `fk_time_blocks_time_blocks1`
    FOREIGN KEY (`corresponding_block`)
